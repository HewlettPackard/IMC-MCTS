...
"""Figure 4b: power breakdown by component.

Grouped stacked bars of Accelerator power (mW) per component, across board sizes
and play strengths (log y). Reads paper/results/fig04_power_breakdown.csv, writes
fig_04_power_breakdown.{png,pdf} into paper/figure/.
"""
...
def load():
    df = pd.read_csv(DATA_DIR / "fig04_power_breakdown.csv")
...
def plot(df):
    ...
    ax.set_ylabel('Power (mW)', fontsize=plotstyle.FONT_SIZES['large'])
    ...
    # Legend keyed by play-strength hatch (not component).
    legend_handles = []
    for i, strength in enumerate(PLAY_STRENGTHS):
        patch = mpatches.Patch(facecolor='white', edgecolor='black',
                               hatch=PATTERNS[i], label=f'{strength} Play Strength')
        legend_handles.append(patch)
    ax.legend(handles=legend_handles, loc='upper center', bbox_to_anchor=(0.5, 1.2),
...
def main():
    print("Generating Figure 4b: Power Breakdown...")
    df = load()
    fig = plot(df)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    plotstyle.save_fig(fig, OUTPUT_DIR / "fig_04_power_breakdown.png", format='png')
    plotstyle.save_fig(fig, OUTPUT_DIR / "fig_04_power_breakdown.pdf", format='pdf')
    print(f"Saved fig_04_power_breakdown.png and .pdf to {OUTPUT_DIR}")

    # Total power range across all play strengths.
    all_totals = []
    ...
    print(f"  Total power range: {min(all_totals):.2f} - {max(all_totals):.2f} mW")
