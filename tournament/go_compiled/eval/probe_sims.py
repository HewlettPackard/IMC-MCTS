...
"""Probe: IMC-MCTS at various sim counts vs GnuGo L10, 10 games each."""
...
def main():
    ...
    print(f"\n{'='*60}", flush=True)
    print(f"RESULTS: IMC-MCTS (strong weights) vs GnuGo-L{GNUGO_LEVEL}", flush=True)
    print(f"{'='*60}", flush=True)
    ...
    with open("probe_sims_results.json", "w") as output_handle:
        json.dump(results, output_handle, indent=2)
    print(f"\nSaved to probe_sims_results.json", flush=True)
